...
clear ECMS_START ECMS_RUN ECMS_RESULT ECMS_DIAGNOSE;
rehash;

helperNames = ["ECMS_START","ECMS_RUN","ECMS_RESULT","ECMS_DIAGNOSE"];
...
grid = uigridlayout(app,[8 1]);
grid.RowHeight = {54,40,58,58,58,58,'1x',34};
...
makeButton("4 · 설치·경로 진단",@(~,~)runAction("진단",@ECMS_DIAGNOSE));

uilabel(grid,"Text",join([ ...
    "배선을 직접 누른 뒤 직각 경로와 꺾임 좌표를 바꿀 수 있습니다."; ...
    "Run은 새 계산을 만들고, Result는 이미 계산된 최신 결과를 엽니다."],newline), ...
