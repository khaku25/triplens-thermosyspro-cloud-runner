...
and actually supplied samples can become evidence. The original files are read
without rewriting them. This module is intentionally local to the Agent API.
"""
...
from .agent_tools import EvidenceStore, DEFAULT_LIMITS, RAW_META_COLUMNS, FORBIDDEN, _event_public, _finite, _clamp_int, _downsample
...
def finite_time(value):
    ...
    if number is None:
        raise ValueError('model_time_s는 유한한 숫자여야 합니다.')
    return number
...
def read_checked(path, kind):
    with Path(path).open(encoding='utf-8-sig', newline='') as f:
        ...
        if len(fields)!=len(set(fields)) or len(normalized)!=len(set(normalized)):
            raise ValueError(f'{kind}: 중복 CSV 열 이름')
        blocked=set(normalized)&(FORBIDDEN|{'test_audit','expected_answer','fault_injection_metadata'})
        if blocked:
            raise ValueError(f'{kind}: 금지된 정답 메타데이터 열: {sorted(blocked)}')
        required={'model_time_s'} | ({'event_id','event_class','equipment','tag'} if kind=='EVENT' else {'record_sequence'})
        if required-set(fields):
            raise ValueError(f'{kind}: 필수 열 누락: {sorted(required-set(fields))}')
        rows=list(reader)
    for row in rows:
        if None in row or any(v is None for v in row.values()):
            raise ValueError(f'{kind}: CSV 열 수 불일치')
        finite_time(row['model_time_s'])
        if kind=='EVENT':
            seq=row.get('event_sequence','0') or '0'
            if _finite(seq) is None or float(seq)!=int(float(seq)):
                raise ValueError('EVENT: 잘못된 event_sequence')
        elif _finite(row.get('record_sequence')) is None:
            raise ValueError('RAW: 잘못된 record_sequence')
    ids=[r.get('event_id') for r in rows] if kind=='EVENT' else [r.get('record_sequence') for r in rows]
    if len(ids)!=len(set(ids)) or any(not x for x in ids):
        raise ValueError(f'{kind}: 근거 식별자 중복 또는 누락')
    return fields,rows
...
class GroundedEvidenceStore(EvidenceStore):
    def __init__(self,event_csv,raw_csv,logic_rows=None,*,event_registry=None,live_tags=None):
        ...
        for key in ('session_id','incident_id'):
            ...
            if len(es)>1 or len(rs)>1 or (es and rs and es!=rs):
                raise ValueError(f'EVENT/RAW {key}가 다르거나 복수입니다. 같은 사고의 파일 쌍을 선택하세요.')
        self.live_tags=set(live_tags or [])
        self.registry={}
        for row in event_registry or []:
            if str(row.get('enabled','1')).lower() not in {'1','true','yes'}: continue
            key=f"{row.get('equipment','').strip()}::{row.get('tag','').strip()}"
            node=str(row.get('source_node','')).strip()
        ...
        self._served={}
        self._raw_original={id(row):i for i,row in enumerate(self.raw_rows,1)}
        ...
        if self.event_rows and self.raw_rows:
            et=[finite_time(r['model_time_s']) for r in self.event_rows]
            rt=list(self._raw_time)
    ...
    def logic_matches(self,tags):
        ...
        for row in self.logic_rows:
            values={str(row.get(k,'')).strip() for k in ('tag_id','canonical_tag','event_tag','source_node')}
            values.update(parts(row.get('linked_tag_ids')))
    ...
    def describe_event(self,row):
        ...
        key=f"{row.get('equipment','').strip()}::{row.get('tag','').strip()}"
        mapped=self.registry.get(key)
        source=str(mapped.get('source_node','')) if mapped else ''
        if not source and row.get('tag') in self.live_tags: source=row['tag']
        item.update(original_tag=row.get('tag',''),lookup_key=key,source_node=source,
                    canonical_tag=source or '',display_name=row.get('message') or key,
                    mapping_status='EQUIPMENT_TAG_EXACT' if mapped else 'EXACT_SOURCE' if source else 'UNREGISTERED_OBSERVATION',
                    raw_available=source in self.raw_tag_inventory,source_kind='EVENT')
        item['logic_ids']=[r.get('logic_id','') for r in self.logic_matches([source]) if source]
        return item

    def publish_event(self,row):
        item=self.describe_event(row)
        self._served[item['evidence_id']]=item
        return item
    ...
    def get_event_window(self,center_time_s,*,before_s=5,after_s=5,limit=30):
        finite_time(center_time_s)
        rows=super().get_event_window(center_time_s,before_s=before_s,after_s=after_s,limit=limit)
        return [self.publish_event(self._events[x['event_id']]) for x in rows]

    def raw_evidence(self,row,tag):
        index=self._raw_original[id(row)]
        eid=f'RAW:{index}:{tag}'
        value=_finite(row.get(tag))
        result={'evidence_id':eid,'source_kind':'RAW','row_number':index,
                'record_sequence':row.get('record_sequence',''),'model_time_s':finite_time(row['model_time_s']),
                'wall_time_utc':row.get('wall_time_utc',''),'tag':tag,'source_node':tag,
                'value':value if value is not None else row.get(tag,''),
                'collector_quality':row.get('collector_quality',row.get('quality','')),
                'logic_ids':[x.get('logic_id','') for x in self.logic_matches([tag])],
                'mapping_status':'EXACT_SOURCE' if tag in self.live_tags else 'UNREGISTERED_OBSERVATION'}
        self._served[eid]=result
        return result

    def get_raw_window(self,tags,start_time_s,end_time_s,*,max_rows=50):
        finite_time(start_time_s); finite_time(end_time_s)
        result=super().get_raw_window(tags,start_time_s,end_time_s,max_rows=max_rows)
        rows=[]
        ...
        result['rows']=rows
        result['requested_tags_truncated']=len(tags)>8
    ...
    def get_tag_series(self,tag,start_time_s,end_time_s,*,max_points=50):
        start,end=sorted([finite_time(start_time_s),finite_time(end_time_s)])
        result=super().get_tag_series(tag,start,end,max_points=max_points)
        ...
        for before,after in zip(valid,valid[1:]):
            ...
            if a in (0,1) and b in (0,1) and a!=b:
                transitions.append({'tag':tag,'from':a,'to':b,'time_interval_s':[float(before['model_time_s']),float(after['model_time_s'])],
                                    'evidence_ids':[self.raw_evidence(before,tag)['evidence_id'],self.raw_evidence(after,tag)['evidence_id']]})
        result['transition_evidence']=transitions[:24]
        result['transitions_truncated']=len(transitions)>24
        if valid:
    ...
    def get_logic_context(self,tags,*,limit=12):
        cap=_clamp_int(limit,12,12)
        resolved={tag:(self.resolve_source(tag) or tag) for tag in tags}
        all_matches=self.logic_matches(list(resolved.values()))
        unregistered=[tag for tag,node in resolved.items() if not self.logic_matches([node])]
        items=[]
        for row in all_matches[:cap]:
            # Public design data only; no scenario/validation answer annotations.
            ...
            linked=parts(row.get('linked_tag_ids'))
            ...
            item['missing_raw_tags']=[t for t in linked if t in self.live_tags and t not in self.raw_tag_inventory]
            items.append(item)
        return {'items':items,'unregistered_tags':unregistered,'resolved_tags':resolved,'infer_unregistered_logic':False,
                'status':'VERIFIED' if tags and not unregistered else 'PARTIAL_OR_UNREGISTERED',
                'verification_scope':'REGISTRATION_ONLY_NOT_INCIDENT_CAUSAL_PROOF','truncated':len(all_matches)>cap,'total_matches':len(all_matches)}

    def get_equipment_state(self,equipment,at_time_s,*,tags=None):
        finite_time(at_time_s)
        out=super().get_equipment_state(equipment,at_time_s,tags=tags)
        ...
        out['raw_sample_time_s']=float(nearest['model_time_s']) if nearest else None
        out['raw_sample_distance_s']=age
        out['raw_stale']=age is None or age>5
    ...
    def upstream_inventory(self):
        nodes={self.describe_event(r)['source_node'] for r in self.event_rows}-{''}
        reached=set(nodes)
        for _ in range(4):
            for row in self.logic_rows:
                outputs=set(parts(row.get('output_nodes_or_tags')))
                if outputs&nodes:
                    reached.update(t for t in parts(row.get('linked_tag_ids')) if t in self.live_tags)
    ...
    def readiness(self):
        ...
        return {'readiness_version':'GENERIC_DUAL_LOG_EVIDENCE_V2','strict_validation_version':VERSION,
                'status':'PASS' if ready else 'COLLECTING','status_scope':'EVIDENCE_READINESS_ONLY',
                'analysis_role':'EVIDENCE_PROVIDER_ONLY','decision_authority':'GEMINI_AGENT','decision_fields_generated_by_python':[],
                'event_rows':len(self.event_rows),'raw_rows':len(self.raw_rows),'validation':self.validation,
                'conclusion':'조회 준비 상태이며 사고 원인 확정이 아닙니다.'}

    def build_agent_bootstrap(self,*,run_id='',data_digest=''):
        boot=super().build_agent_bootstrap(run_id=run_id,data_digest=data_digest)
        boot['incident_id']='OPAQUE_INCIDENT'  # user-chosen incident labels must not hint at the answer
        boot.update(evidence_version=VERSION,validation=self.validation,evidence_readiness=self.readiness(),
                    raw_tag_inventory=self.raw_tag_inventory,upstream_available_tags=self.upstream_inventory(),
                    initial_events=[self.publish_event(r) for r in self.event_rows[:12]])
