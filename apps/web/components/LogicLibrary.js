...
export function LogicLibraryDialog() {
  ...
  return (
    <dialog ref={ref} aria-label="태그·로직 상세보기" onCancel={close}
      ...
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', background: '#17364d', color: 'white', minHeight: 48 }}>
      ...
      {selection ? <iframe key={selection.revision} title="태그와 draw.io 로직 도면" src={src} style={{ width: '100%', height: 'calc(100% - 54px)', border: 0, display: 'block' }} /> : null}
    </dialog>
  );
}
