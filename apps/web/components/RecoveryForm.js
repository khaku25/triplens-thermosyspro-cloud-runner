'use client';
import {normalizeRecovery,recoveryStatusLabel,validateRecovery,workflowLabel} from '../lib/recoveryModel.mjs';
...
export default function RecoveryForm({value,onChange,catalog=[]}){
  ...
  return <section className="recovery-form" aria-label="복구 기록 입력">
    ...
    <div className="recovery-approval"><button className="export-button" type="button" disabled={!validation.valid||!validation.recovery.approver||Boolean(recovery.approved_at)} onClick={approve}>승인 기록</button><span>{recovery.approved_at?`승인 기록 시각: ${recovery.approved_at}`:'필수 정보와 기록상 승인자를 입력한 뒤 명시적으로 승인 기록을 남기세요.'}</span></div>
  </section>;
}
