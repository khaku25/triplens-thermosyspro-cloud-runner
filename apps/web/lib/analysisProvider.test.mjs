...
test('model choices reflect server key availability without exposing secrets',()=>{
  const choices=analysisProviderChoices({analysis_models:[
    ...
    {id:'openai',label:'GPT-6 Luna',model:'gpt-6-luna',available:false},
  ]});
  ...
  assert.deepEqual(choices.map(({id,label,disabled,displayLabel})=>({id,label,disabled,displayLabel})),[
    ...
    {id:'openai',label:'GPT-6 Luna',disabled:true,displayLabel:'GPT-6 Luna · API 키 설정 필요'},
  ]);
  assert.equal(analysisProviderLabel('openai',{analysis_models:[{id:'openai',label:'GPT-6 Luna',model:'gpt-6-luna',available:true}]}),'GPT-6 Luna');
});
...
test('older API contracts keep Gemini usable while Luna stays unavailable',()=>{
  ...
  assert.equal(choices[1].label,'GPT-6 Luna');
  assert.equal(choices[1].model,'gpt-6-luna');
