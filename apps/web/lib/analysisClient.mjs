...
import {mergeEvidenceCatalog,preferredSourceTag} from './reportIntegrity.mjs';
...
export function humanReviewNote(value){
 ...
 const translations=[
  ['근거 ID 미연결','원본 기록 미연결 · 추가 확인 필요'],
  ['설명 미제공','분석 설명 없음 · 담당자 확인 필요'],
  ['인용 근거에 없는 태그:','원본 관측 및 주장-근거 연결 대조:'],
  ['미조회 또는 존재하지 않는 근거 ID:','참조 원본 기록 재확인:'],
  ['주장한 시간구간과 인용 표본의 시각 불일치','관측 구간 경계 표본 및 보고서 시간 범위 대조 필요'],
  ['태그별 관측 구간의 경계 표본 인용 누락:','관측 구간 경계 표본 추가 연결 필요:'],
  ['AI 시각과 인용 근거 시각 불일치','분석 시각-원본 기록 시각 대조 필요'],
  ['RAW Tool 조회 성공 기록 없음','공정 시계열 조회 미확인 · 원본 RAW 대조 필요'],
  ['Logic Tool 조회 성공 기록 없음','관련 로직 조회 미확인 · 승인 로직 원장 대조 필요'],
  ['Tool 오류 또는 호출 예산 소진','근거 조회 미완료 · 추가 확인 필요'],
  ['Primary Cause 미확인','선행 원인 확정 근거 부족'],
  ['Direct Trigger 미확인','직접 보호동작 판단 근거 부족'],
  ['Direct Trigger의 등록 로직 연결 미확인','직접 보호동작-승인 로직 연결 미확인']
 ];
...
export function buildDraftRows(envelope,recoveryRows=[]){
 ...
 const evidence=mergeEvidenceCatalog(events,envelope?.evidence_catalog||[]);
