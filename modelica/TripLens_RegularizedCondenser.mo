within ;
model TripLens_RegularizedCondenser
  ...
  parameter Modelica.SIunits.MassFlowRate coolantFlowRegularization=1
    "Smooth low-flow scale; not a fictitious bypass flow";
  parameter Modelica.SIunits.SpecificHeatCapacity coolantSpecificHeat=4180
...
public
  ...
  Modelica.SIunits.MassFlowRate coolantFlowMagnitude
    "Regularized magnitude that becomes exactly zero at zero actual flow";
  Modelica.SIunits.TemperatureDifference coolantTemperatureDriving;
  Modelica.SIunits.Power steamHeatDemand;
  Modelica.SIunits.Power coolantHeatCapacity;
  Modelica.SIunits.Power effectiveHeatRemoval;
  Modelica.SIunits.Power coolantEnergyTransport;
  Modelica.SIunits.Power heatBalanceRegularizationError;
...
equation
  ...
  /* Thermal power exchanged from steam to cooling water.
     The upstream model forces all incoming latent heat through the cooling
     side even when Cee.Q tends to zero. That makes the coolant outlet
     enthalpy singular. Here the removable heat is limited continuously by
     the actual CW thermal capacity. */
  steamHeatDemand = noEvent(max(0, Cv.Q*(Cv.h - hl)));
  coolantFlowMagnitude = noEvent(sqrt(Cee.Q^2 +
    coolantFlowRegularization^2) - coolantFlowRegularization);
...
end TripLens_RegularizedCondenser;
