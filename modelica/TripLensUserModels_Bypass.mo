...
package TripLensUserModels_Bypass
  "Writable user package for the TripLens combined-cycle model"


model CombinedCycle_TripTAC_Bypass
  "CCPP model with visible physical HP and LP turbine bypass systems"
  // TRIPLENS_V42_VISIBLE_TURBINE_BYPASS_PATCH_V1
  model VPPPressureDrivenBypassValve
    ...
    parameter Real rhoNom(unit="kg/m3")=10
  ...
  annotation(Icon(coordinateSystem(preserveAspectRatio=false), graphics={
      Polygon(points={{-80,50},{0,0},{-80,-50},{-80,50}},
        lineColor={0,95,191}, fillColor={255,255,255},
        fillPattern=FillPattern.Solid),
      Polygon(points={{80,50},{0,0},{80,-50},{80,50}},
        lineColor={0,95,191}, fillColor={255,255,255},
        fillPattern=FillPattern.Solid),
      Line(points={{0,0},{0,70}}, color={0,95,191}, thickness=1),
      Text(extent={{-50,100},{50,72}}, textString="BP",
        textColor={0,95,191})}));
  ...
  equation
    ...
    C1.h = C1.h_vol;
  annotation(Icon(coordinateSystem(preserveAspectRatio=false), graphics={
      Ellipse(extent={{-55,55},{55,-55}}, lineColor={0,127,255},
        fillColor={170,213,255}, fillPattern=FillPattern.Solid),
      Line(points={{-80,0},{80,0}}, color={0,95,191}, thickness=1),
      Polygon(points={{80,0},{50,18},{50,-18},{80,0}},
        lineColor={0,95,191}, fillColor={0,95,191},
        fillPattern=FillPattern.Solid)}));
  ...
  parameter Real vppSpraySeatLeak(unit="kg/s") = 0
  ...
  parameter Real vppHPHeaderVolume(unit="m3") = 1
    "Preliminary cold-reheat mixing volume";
  parameter Real vppLPHeaderVolume(unit="m3") = 50
    "Preliminary condenser-inlet steam mixing volume";
  parameter Real vppHPMainFlow0(unit="kg/s") = 151.7690991976083
    "Verified pre-Trip HP steam-flow initialization point";
  parameter Real vppIPMainFlow0(unit="kg/s") = 176.7893383342879
    "Verified pre-Trip hot-reheat steam-flow initialization point";
  parameter Real vppCondenserSteamFlow0(unit="kg/s") =
      196.6524916480812
    "Verified pre-Trip condenser steam-flow initialization point";
  parameter Real vppHPSteamDensity0(unit="kg/m3") = 34
    "Normal HP-main-steam density used for Trip-transient regularization";
  parameter Real vppHotReheatSteamDensity0(unit="kg/m3") = 6.5
  ...
  Real vppHPBypassMassFlow(unit="kg/s");
  Real vppLPBypassMassFlow(unit="kg/s");
  Real vppHPSprayMassFlow(unit="kg/s");
  Real vppLPSprayMassFlow(unit="kg/s");
  Real vppHPBypassInletPressure(unit="Pa");
  Real vppLPBypassInletPressure(unit="Pa");
  Real vppHPBypassOutletPressure(unit="Pa");
  Real vppLPBypassOutletPressure(unit="Pa");
  Real vppHPBypassInletTemperature(unit="K");
  Real vppLPBypassInletTemperature(unit="K");
  Real vppHPBypassOutletTemperature(unit="K");
  Real vppLPBypassOutletTemperature(unit="K");
  Real vppCondenserPressure(unit="Pa");
  Real vppCondenserLevel(unit="m");
  ...
  ThermoSysPro.WaterSteam.Junctions.Splitter2 vppHPSplitter(
    ...
    Cs2(Q(start=0, nominal=200),
        h(start=3248547.5), h_vol(start=3450835))) annotation(
    Placement(transformation(extent={{-190,-240},{-170,-220}})));
  VPPPressureDrivenBypassValve vppHPBypassValve(
    ...
    C2(P(start=2726700), Q(start=0, nominal=200),
       h(start=3248547.5), h_vol(start=3046260))) annotation(
    Placement(transformation(extent={{-120,-210},{-80,-190}})));
  ThermoSysPro.WaterSteam.BoundaryConditions.SourceQ vppHPSpraySource(
    Q0=vppSpraySeatLeak, h0=1396866) annotation(
    Placement(transformation(extent={{-74,-170},{-44,-140}})));
  VPPFixedFlowInjector vppHPSprayInjector annotation(
    Placement(transformation(extent={{-34,-162},{-14,-142}})));
  ThermoSysPro.InstrumentationAndControl.Connectors.OutputReal
    vppHPSprayFlowCommand annotation(
    Placement(transformation(extent={{-112,-160},{-92,-140}})));
  ThermoSysPro.WaterSteam.Volumes.VolumeC vppHPColdReheatVolume(
    ...
    Cs(Q(start=vppHPMainFlow0, nominal=200),
       h(start=3046260), h_vol(start=3046260))) annotation(
    Placement(transformation(extent={{20,-216},{40,-196}})));
  ...
  ThermoSysPro.WaterSteam.Junctions.Splitter2 vppLPSplitter(
    ...
    Cs1(Q(start=vppIPMainFlow0, nominal=200),
        h(start=3523910), h_vol(start=3523910)),
    Cs2(Q(start=0, nominal=200),
        h(start=2962470), h_vol(start=3523910))) annotation(
    Placement(transformation(extent={{-190,-324},{-170,-304}})));
  VPPPressureDrivenBypassValve vppLPBypassValve(
    ...
    C2(P(start=6136), Q(start=0, nominal=200),
       h(start=2962470), h_vol(start=2401030))) annotation(
    Placement(transformation(extent={{320,-338},{360,-318}})));
  ThermoSysPro.WaterSteam.BoundaryConditions.SourceQ vppLPSpraySource(
    Q0=vppSpraySeatLeak, h0=550000) annotation(
    Placement(transformation(extent={{390,-380},{420,-350}})));
  VPPFixedFlowInjector vppLPSprayInjector annotation(
    Placement(transformation(extent={{430,-372},{450,-352}})));
  ThermoSysPro.InstrumentationAndControl.Connectors.OutputReal
    vppLPSprayFlowCommand annotation(
    Placement(transformation(extent={{360,-372},{380,-352}})));
  ThermoSysPro.WaterSteam.Volumes.VolumeC vppCondenserSteamVolume(
    ...
    Cs(Q(start=vppCondenserSteamFlow0, nominal=200),
       h(start=2401030), h_vol(start=2401030))) annotation(
    Placement(transformation(extent={{626,-286},{646,-266}})));
...
equation
  ...
  connect(perteChargeK1.C2, vppCondenserSteamVolume.Ce1) annotation(
    Line(points={{627,-230},{636,-230},{636,-266}}, color={255,0,0}));
  connect(vppCondenserSteamVolume.Cs, CapteurDebitVapCondenseur.C1) annotation(
    Line(points={{646,-276},{657,-276},{657,-254}}, color={255,0,0}));
  ...
  connect(DoubleDebitHP.Cs, vppHPSplitter.Ce) annotation(
    Line(points={{-325,-110},{-325,-230},{-190,-230}}, color={255,0,0}));
  connect(vppHPSplitter.Cs1, vanne_entree_TurbineHP.C1) annotation(
    Line(points={{-170,-230},{-157,-230}}, color={255,0,0}));
  connect(vppHPSplitter.Cs2, vppHPBypassValve.C1) annotation(
    Line(points={{-180,-220},{-180,-200},{-120,-200}}, color={0,127,255}, thickness=1));
  ...
  connect(DoubleDebitMP.Cs, vppLPSplitter.Ce) annotation(
    Line(points={{-265,-110},{-265,-314},{-190,-314}}, color={255,0,0}));
  connect(vppLPSplitter.Cs1, vanne_entree_TurbineMP.C1) annotation(
    Line(points={{-170,-314},{-157,-314}}, color={255,0,0}));
  connect(vppLPSplitter.Cs2, vppLPBypassValve.C1) annotation(
    Line(points={{-180,-324},{-180,-328},{320,-328}}, color={0,127,255}, thickness=1));
  ...
  connect(TurbineHP.Cs, vppHPColdReheatVolume.Ce1) annotation(
    Line(points={{5,-230},{12,-230},{12,-206},{20,-206}}, color={255,0,0}));
  connect(vppHPColdReheatVolume.Cs, MoitieDebitHP.Ce) annotation(
    Line(points={{40,-206},{50,-206},{50,-170},{81,-170}}, color={255,0,0}));
...
model CombinedCycle_TripTAC_Bypass_OMCStart
  ...
  extends CombinedCycle_TripTAC_Bypass(
    Kin_SMP2(fixed=true, start=10),
    BallonHP(steady_state=false, use_start_enthalpies=true,
      P0=12726424.235625941, hl0=1460508.128907675,
      hv0=2664791.3052738607, Vf0=0.5, zl(fixed=false)),
    BallonMP(steady_state=false, use_start_enthalpies=true,
      P0=2733918.2848144253, hl0=980960.1562978515,
      hv0=2798761.3254371085, Vf0=0.5, zl(fixed=false)),
    BallonBP(steady_state=false, use_start_enthalpies=true,
    ...
    vanne_vapeurMP(p_rho=14),
    PompeAlimMP(p_rho=931.2517020786314,
      Q(start=12.5101195684555)),
    Vanne_alimentationMPHP2(p_rho=931.2517020786314,
      Q(start=12.5101195684555)),
    ...
    perteChargeK2(p_rho=990.3699122094005),
    VolumePreTHP(steady_state=false, P0=12431000,
      h0=3432930.9918569606),
    MelangeurPreTMP(steady_state=false, P0=2548500,
    ...
    TurbineMP(Cs(h(start=3029367.6706168973))),
    EvaporateurHP(
