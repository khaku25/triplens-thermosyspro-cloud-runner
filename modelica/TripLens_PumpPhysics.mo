...
package TripLens_PumpPhysics
  ...
  model SpringLoadedCheckValve
    "Finite-resistance non-return valve with spring-equivalent closing flow"
    ...
    parameter Real closedResistance(unit="Pa.s/kg") = 1e8
  ...
  equation
    assert(openResistance > 0 and closedResistance > openResistance,
    ...
    // A single finite-resistance equation keeps the algebraic structure fixed
    // at the switching event. The closed resistance limits reverse leakage
    // without the divide-by-zero generated by an ideal equation switch.
    deltaP = (if ouvert then openResistance else closedResistance)*Q;
