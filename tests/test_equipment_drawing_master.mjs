...
test('equipment drawing master covers every enabled EVENT equipment exactly',()=>{
  ...
  const master=parseCsv(fs.readFileSync(new URL('../config/equipment_drawing_master_v1.csv',import.meta.url),'utf8'));
  assert.equal(equipment.length,40);
  assert.equal(EQUIPMENT_DRAWING_MASTER.length,master.length);
  for(const row of master) assert.deepEqual(resolveEquipmentDrawing(row.equipment_id),row);
