...
def main():
    ...
    with sync_playwright() as p:
        ...
        for width,height,name in [(1440,1000,'desktop'),(390,844,'mobile')]:
            ...
            page.locator('summary',has_text='Propagation · 파급 과정').click()
            ...
            expect(page.get_by_text(re.compile('Run ID: RUN-UI-REGRESSION'))).to_be_hidden()
            page.locator('summary',has_text='감사·버전 정보').click()
            ...
            expect(page.get_by_role('button',name='태그 기준 도면 · vpp52GTClosed',exact=True)).to_be_visible()
            rule_button=page.get_by_role('button',name='운전조건·룰 기준 도면 · SEQ-52GT-OPEN',exact=True)
            ...
            page.locator('summary',has_text='전체 SOE 보기').click()
