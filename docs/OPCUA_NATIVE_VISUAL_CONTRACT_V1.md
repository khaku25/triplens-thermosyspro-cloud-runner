...
- `topology/opcua/check_valves/*.svg`: HP/IP/LP FWP discharge check-valve
  drawings and the complete feedwater discharge wiring view. Each passive
  valve binds seven solved READ nodes: open state, opening, mass flow,
  differential pressure, inlet pressure, outlet pressure and effective
...
python3 -m unittest tests.test_all_fwp_check_valves -v
```
