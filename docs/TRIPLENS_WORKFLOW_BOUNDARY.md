...
현재 등록된 물리 어댑터는 GT 배기경계 변화와 펌프 물리 Trip이다. 펌프 행렬
실행은 HP/IP/LP 경로를 하나씩 동적 원심펌프로 교체해 격리 검증하고, 복수·LP
공용 경로와 CW 경계를 명시적으로 구분한다. 원본 모델에 존재하지 않는 RECIRC
전동기 펌프는 만들어내지 않는다. 새 사고는 해당 설비의 물리 어댑터를 추가한 뒤
같은 RAW-only 출력 계약을 쓴다.
