...
Recovery is a separate human record with status, recovery time, operator, actions, restart conditions, evidence IDs and approver. Stored status values are `UNKNOWN`, `PARTIAL`, `RECOVERED`, and `NOT_RECOVERED`; the UI separately distinguishes `입력 대기` (no human record), `복구 입력 미완료`, `승인 대기`, and `승인 완료`. Those workflow labels are not aliases for the engineering value `UNKNOWN`. Cause Verification Gate and recovery approval remain independent, and the document stays a draft until both are complete.
