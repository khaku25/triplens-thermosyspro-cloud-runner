...
Status: **topology locked; V10 physical implementation provisional; plant validation pending**
...
Each implemented bypass train contains a one-way
`VPPPressureDrivenBypassValve`, a commanded spray-water `SourceQ`, a finite
mixing/header `VolumeC`, mass-flow output, and inlet/outlet pressure and
temperature outputs. The custom valve isolates a fully closed branch from the
upstream IF97 initialization loop. The header volumes retain finite thermal
hold-up without adding a duplicate ideal-node pressure state.
...
The V10 engineering values are `vppHPBypassCvmax=1890` and
`vppLPBypassCvmax=22000`. They are preliminary Modelica calibration values,
not approved settings. ThermoSysPro calculates flow from pressure loss,
density, valve position, and `Cv`; final acceptance still requires a complete
post-Trip transient and mass/energy balance in the pinned OpenModelica version.
...
## 8. Editable model and final circuit package

The public design package is generated by
`scripts/package_turbine_bypass_design.py`. It applies the deterministic V10
patch to the pinned upstream `CombinedCycle_TripTAC`, checks every registered
process/control connection, and exports:

| File | Purpose |
|---|---|
| `CombinedCycle_TripTAC.mo` | Full patched ThermoSysPro model at the original class path |
| `TripLens_CombinedCycle_TripTAC.mo` | Parameterized VPP simulation wrapper |
| `TripTAC_Turbine_Bypass_Final.svg` | Editable 1920×1080 process and Trip-wiring circuit |
| `turbine_bypass_connections_v1.csv` | Exact Modelica connection/equation evidence |
| `design-manifest.json` | Source commit, patch marker, topology flags, and SHA-256 values |

The repository source drawing remains
[`topology/turbine_bypass_vpp.svg`](../topology/turbine_bypass_vpp.svg). Each
drawn process or control path carries a `data-connection-id` matched to
[`config/turbine_bypass_connections_v1.csv`](../config/turbine_bypass_connections_v1.csv),
so the drawing cannot silently diverge from the generated model.
